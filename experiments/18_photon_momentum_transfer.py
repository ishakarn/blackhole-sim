"""v1.8 full 3D Schwarzschild renderer with improved photon-momentum transfer shading."""
...
OUTPUT_DIR = pathlib.Path(__file__).parent.parent / "outputs" / "figures" / "18_photon_momentum_transfer"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Full 3D Schwarzschild render with improved photon-momentum transfer shading."
    )
    ...
    parser.add_argument("--transfer-mode", choices=["tangent", "momentum"], default="momentum")
...
def main() -> None:
    ...
    g_momentum, momentum_state = momentum_transfer_g_factor(
        hit_position,
        hit_direction,
        hit_radius.clamp(min=args.disk_inner_radius),
        disk_mask,
    ...
    g_factor = g_momentum if args.transfer_mode == "momentum" else g_tangent
    ...
    g_momentum_out = downsample_image(scalar_to_rgb(g_momentum, disk_mask), supersample, width, height).clamp(0.0, 1.0)
    g_tangent_out = downsample_image(scalar_to_rgb(g_tangent, disk_mask), supersample, width, height).clamp(0.0, 1.0)
    g_diff_out = downsample_image(scalar_to_rgb((g_momentum - g_tangent).abs(), disk_mask), supersample, width, height).clamp(0.0, 1.0)
    ...
    output_path = pathlib.Path(args.output) if args.output else OUTPUT_DIR / "full3d_momentum_transfer_render.png"
    save_tensor_image(final_image, output_path)
    save_tensor_image(raw_image, OUTPUT_DIR / "full3d_momentum_transfer_raw.png")
    save_tensor_image(emissivity_out, OUTPUT_DIR / "full3d_momentum_transfer_emissivity.png")
    save_tensor_image(g_momentum_out, OUTPUT_DIR / "full3d_momentum_transfer_g_factor.png")
    save_tensor_image(g_tangent_out, OUTPUT_DIR / "full3d_transfer_g_factor_tangent.png")
    save_tensor_image(g_momentum_out, OUTPUT_DIR / "full3d_transfer_g_factor_momentum.png")
    save_tensor_image(g_diff_out, OUTPUT_DIR / "full3d_transfer_g_factor_difference.png")
    save_tensor_image(intensity_out, OUTPUT_DIR / "full3d_momentum_transfer_intensity.png")
    save_tensor_image(status_out, OUTPUT_DIR / "full3d_momentum_transfer_status_map.png")
    ...
    stats = transfer_difference_stats(g_tangent, g_momentum, disk_mask)
    print(
        f"Preset: {args.preset}, resolution={width}, supersample={supersample}, transfer_mode={args.transfer_mode}"
    )
    print(
    ...
    print(
        f"g tangent: min={stats['tangent_min']:.4f}, max={stats['tangent_max']:.4f}, mean={stats['tangent_mean']:.4f}"
    )
    print(
        f"g momentum: min={stats['momentum_min']:.4f}, max={stats['momentum_max']:.4f}, mean={stats['momentum_mean']:.4f}"
    )
    print(
        f"g difference: mean_abs={stats['mean_abs_diff']:.4f}, max_abs={stats['max_abs_diff']:.4f}"
    )
    ...
    if args.show:
        ...
        panels = [
            ...
            (g_tangent_out, "g Tangent"),
            (g_momentum_out, "g Momentum"),
            (g_diff_out, "|g Momentum - g Tangent|"),
            (intensity_out, "Observed Intensity"),
            (final_image, "Momentum Transfer Render"),
        ]
