"""v1.4 full 3D Schwarzschild renderer with relativistic disk appearance proxies."""
...
OUTPUT_DIR = pathlib.Path(__file__).parent.parent / "outputs" / "figures" / "14_relativistic_disk_effects"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Full 3D Schwarzschild renderer with cleaned disk compositing and relativistic disk proxies."
    )
    ...
    parser.add_argument("--edge-width", type=float, default=3.0)
    parser.add_argument("--inner-edge-boost", type=float, default=1.5)
...
def scalar_to_rgb(values: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    rgb = values.clamp(0.0, 1.0).unsqueeze(-1).repeat(1, 1, 3)
    return rgb * mask.unsqueeze(-1)
...
def build_disk_maps(
    *,
    ...
    if edge_width > 0.0:
        edge_falloff = 1.0 - smoothstep(disk_outer_radius - edge_width, disk_outer_radius, radius)
        inner_boost_profile = 1.0 - smoothstep(disk_inner_radius, disk_inner_radius + edge_width, radius)
    else:
        edge_falloff = torch.ones_like(radius)
        inner_boost_profile = torch.ones_like(radius)

    emissivity = emissivity * (1.0 + (inner_edge_boost - 1.0) * inner_boost_profile)
    emissivity = emissivity.clamp(0.0, 1.5) * disk_mask
    ...
    beaming_factor = torch.nan_to_num(beaming_factor, nan=0.0)
    redshift_factor = torch.nan_to_num(redshift_factor, nan=0.0)
    combined_intensity = emissivity * beaming_factor * redshift_factor
    ...
    clean_emission = color * combined_intensity.unsqueeze(-1) * disk_mask.unsqueeze(-1)

    opacity = (disk_alpha * edge_falloff).clamp(0.0, 1.0) * disk_mask

    return {
        "clean_emission": clean_emission.clamp(0.0, 1.0),
        "opacity": opacity,
...
def main() -> None:
    ...
    captured_mask = torch.from_numpy(result.status_map == STATUS_CAPTURED).to(device=device)
    ...
    opacity = disk_maps["opacity"].unsqueeze(-1)
    clean_emission = disk_maps["clean_emission"]
    disk_composite = clean_emission * opacity + direct_background * (1.0 - opacity)
    image = torch.where(disk_mask.unsqueeze(-1), disk_composite, image)
    ...
    final_internal = apply_bloom(raw_image, clean_emission, args.bloom_strength, args.bloom_radius) if args.bloom else raw_image

    final_image = downsample_image(final_internal, supersample, width, height).clamp(0.0, 1.0)
    clean_emission_out = downsample_image(clean_emission, supersample, width, height).clamp(0.0, 1.0)
    beaming_out = downsample_image(scalar_to_rgb(disk_maps["beaming_factor"], disk_mask.float()), supersample, width, height).clamp(0.0, 1.0)
    redshift_out = downsample_image(scalar_to_rgb(disk_maps["redshift_factor"], disk_mask.float()), supersample, width, height).clamp(0.0, 1.0)
    opacity_out = downsample_image(scalar_to_rgb(disk_maps["opacity"], disk_mask.float()), supersample, width, height).clamp(0.0, 1.0)
    status_out = downsample_image(make_status_rgb(result.status_map).to(device=device), supersample, width, height).clamp(0.0, 1.0)

    output_path = pathlib.Path(args.output) if args.output else OUTPUT_DIR / "full3d_relativistic_disk.png"
    save_tensor_image(final_image, output_path)
    save_tensor_image(clean_emission_out, OUTPUT_DIR / "full3d_disk_clean_emission.png")
    save_tensor_image(beaming_out, OUTPUT_DIR / "full3d_disk_beaming_factor.png")
    save_tensor_image(redshift_out, OUTPUT_DIR / "full3d_disk_redshift_factor.png")
    save_tensor_image(opacity_out, OUTPUT_DIR / "full3d_disk_opacity.png")
    save_tensor_image(status_out, OUTPUT_DIR / "full3d_status_map.png")
    if args.bloom:
        raw_out = downsample_image(raw_image, supersample, width, height).clamp(0.0, 1.0)
        save_tensor_image(raw_out, OUTPUT_DIR / "full3d_relativistic_disk_raw.png")
    ...
    if args.show:
        ...
        panels = [
            ...
            (clean_emission_out, "Clean Emission"),
            (beaming_out, "Beaming"),
            (redshift_out, "Redshift"),
            (opacity_out, "Opacity"),
            (final_image, "Relativistic Disk"),
        ]
